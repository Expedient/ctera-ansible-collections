...
from ansible_collections.ctera.ctera.plugins.module_utils.ctera_filer_base import (
    CteraFilerBase,
)
...
class CteraFilerShare(CteraFilerBase):
            ...
            dict(
                ...
                acl=dict(
                    ...
                    options=dict(
                        ...
                        perm=dict(
                            required=True, choices=["ReadWrite", "ReadOnly", "None"]
                        ),
                    ),
                ),
                access=dict(
                    required=False,
                    choices=["winAclMode", "authenticated"],
                    default="winAclMode",
                ),
                csc=dict(
                    required=False,
                    choices=["manual", "documents", "disable"],
                    default="manual",
                ),
    ...
    def _add_share(self):
        add_params = {
            k: v for k, v in self.parameters.items() if k in CteraFilerShare._add_params
        }
        ...
        if add_params.get("trusted_nfs_clients") is not None:
            add_params["trusted_nfs_clients"] = [
                self._make_NFSv3AccessControlEntry(trusted_nfs_clients_entry)
                for trusted_nfs_clients_entry in add_params["trusted_nfs_clients"]
            ]
        self._ctera_filer.shares.add(**add_params)
        self.ansible_module.ctera_return_value().changed().msg("Share created").put(
        ...
        if modified_attributes:
            if modified_attributes.get("acl") is not None:
                modified_attributes["acl"] = [
                    self._make_ShareAccessControlEntry(acl_entry)
                    for acl_entry in modified_attributes["acl"]
                ]
            if modified_attributes.get("trusted_nfs_clients") is not None:
                modified_attributes["trusted_nfs_clients"] = [
                    self._make_NFSv3AccessControlEntry(trusted_nfs_clients_entry)
                    for trusted_nfs_clients_entry in modified_attributes[
                        "trusted_nfs_clients"
                    ]
                ]
            self._ctera_filer.shares.modify(
            ...
            self.ansible_module.ctera_return_value().skipped().msg(
                "Share details did not change"
            ).put(name=self.parameters["name"])

    def _ensure_absent(self, share):
        if share:
            self._ctera_filer.shares.delete(self.parameters["name"])
            self.ansible_module.ctera_return_value().changed().msg("Share deleted").put(
                name=self.parameters["name"]
            )
        else:
            self.ansible_module.ctera_return_value().skipped().msg(
    ...
    def _to_share_dict(share_obj):
        ...
        share_dict["directory"] = os.path.join(
            share_obj.volume, share_obj.directory[1:]
        )
        share_dict["acl"] = [
            CteraFilerShare._to_acl_dict(acl_entry) for acl_entry in share_obj.acl
        ]
        ...
        share_dict["trusted_nfs_clients"] = [
            CteraFilerShare._to_trusted_nfs_clients_dict(trusted_nfs_clients_entry)
            for trusted_nfs_clients_entry in share_obj.trustedNFSClients
        ]
        return share_dict
    ...
    def _to_acl_dict(acl_obj):
        ...
        acl_dict[
            "principal_type"
        ] = acl_obj.principal2._classname  # pylint: disable=protected-access
        if acl_dict["principal_type"] in [
            gateway_enum.PrincipalType.LU,
            gateway_enum.PrincipalType.LG,
        ]:
